...
class ResourceRequest(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if previous is not None and previous.total_units is not None:
        ...
        if resource.stock == 0:
            resource.status = settings.ITEM_STATUS_COMPLETE
        if request.stock == 0:
            request.status = settings.ITEM_STATUS_COMPLETE
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        self.resource.status = settings.ITEM_STATUS_VERIFIED
        self.request.status = settings.ITEM_STATUS_VERIFIED
    ...
    def _restore_amounts_if_connections_changed(self, previous):
        if previous.resource.id != self.resource.id:
            ...
            previous.resource.status = settings.ITEM_STATUS_VERIFIED
            previous.resource.save()
        elif previous.request.id != self.request.id:
            previous.request.stock += previous.total_units
            previous.request.status = settings.ITEM_STATUS_VERIFIED
